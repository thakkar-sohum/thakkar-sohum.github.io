...
class TracerPerson {
    ...
    interact(otherPerson) {
        ...
        if (this.covidState == "tracer" && otherPerson.covidState == "infected") {
...
class TracerWorldSimulation extends ParticleSimulation {
    static states = ["latent", "asymptomatic", "infected", "susceptible", "recovered", "dead", "tracer", "traced"];

    constructor(dimensions, duration = 1000, ticksPerDay = 10, numPersons = 20, numTracers = 5) {
    ...
    addPersons() {
        for (let i = 0; i < this.numPersons-this.numTracers; i++){ this.addPerson(); }
        // console.log(this.people)
        for (let i = 0; i < this.numTracers; i++){ this.addTracer(); }
    }

    addPerson(){
        var person = new TracerPerson({
